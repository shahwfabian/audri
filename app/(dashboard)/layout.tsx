          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                style={{
                  background: active ? "var(--gold-10)" : item.flagship ? "rgba(201,168,76,0.05)" : "transparent",
                ...
                onMouseLeave={e => {
                  if (!active) {
                    (e.currentTarget as HTMLElement).style.background = item.flagship ? "rgba(201,168,76,0.05)" : "transparent";
        ...
        <div className="h-16 flex items-center px-6 sticky top-0 z-30" style={{ background: "rgba(8,8,8,0.90)", borderBottom: "1px solid var(--border)", backdropFilter: "blur(8px)" }}>
          ...
          <div className="flex items-center gap-3">
            ...
            {!onboardingComplete && (
              <Link
                href="/onboarding"
                className="text-xs px-3 py-1.5 rounded-lg font-medium transition-colors badge-amber"
              >
                Complete onboarding →
              </Link>
            )}

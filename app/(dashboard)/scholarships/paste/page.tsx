          ...
          <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "1rem", padding: "1.5rem" }}>
            ...
            <div className="flex flex-wrap gap-2">
              {parsed.categories.map((cat) => (
                <span key={cat} className="text-xs px-2.5 py-1 rounded-full" style={{ background: "rgba(201,168,76,0.10)", color: "var(--gold-dark)", border: "1px solid rgba(201,168,76,0.2)" }}>{cat}</span>
              ))}
            </div>
              ...
              {showPrompts && (
                <div className="px-5 pb-5 space-y-3" style={{ borderTop: "1px solid var(--border)" }}>
                  {parsed.prompts.map((p, i) => (
                    <div key={p.id ?? i} className="rounded-xl p-4 mt-4" style={{ background: "rgba(201,168,76,0.08)", border: "1px solid rgba(201,168,76,0.2)" }}>

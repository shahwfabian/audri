...
const CATEGORY_STYLES: Record<StoryCategory, { bg: string; color: string }> = {
  ...
  FINANCIAL_HARDSHIP:   { bg: "rgba(201,168,76,0.15)",   color: "var(--gold)" },
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          {story.bestUseCases.map((use) => (
            <span key={use} className="text-xs px-2 py-0.5 rounded-full" style={{ background: "rgba(201,168,76,0.10)", color: "var(--gold-dark)", border: "1px solid rgba(201,168,76,0.2)" }}>
              {use}
            </span>
          ))}
        </div>
